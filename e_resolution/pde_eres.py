...
def calculate_energy_resolution(hist_target, key, hist_prev=None, breakdown=False):
    ...
    if key == "max":
        ...
        pde = 0.47
    return np.sqrt(mean * pde * (1-pde)) / (mean * pde) 
...
for s_file in special_files:
    ...
    resolutions = calculate_energy_resolution(f.Get("hist_PTE"), key, f.Get("hist_PTE"), True)
...
for i,file in enumerate(root_files):
    ...
    pde = pde_list[index]
    energy_resolution_pde = np.sqrt(mean_pte * pde * (1-pde)) / (mean_pte * pde)
...
# Plotting
plt.scatter(over_voltages, energy_resolutions_pde, label='PDE',s=size_marker, marker='o', color='mediumblue')
...
plt.grid(True)
plt.savefig("energy_res_pde.pdf")
print(root_files)
