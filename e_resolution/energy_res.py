...
def extract_over_voltage(filename):
    print(round(float(filename.split('_')[1].replace('.root', '')),1))
    if round(float(filename.split('_')[1].replace('.root', '')),1) == 6.2:
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
...
for i,file in enumerate(root_files):
    ...
    energy_resolution_ct_dec = np.sqrt((sigma_ct * mean_pde/mean_ct)**2 - sigma_pde**2) / mean_ct
...
plt.scatter(over_voltages, energy_resolutions_pde, label='LS+PTE+PDE',s=size_marker, marker='o', color='red')
...
plt.scatter(over_voltages, energy_resolutions_charge, label='LS+PTE+PDE+DCR+CT+AP',s=size_marker, marker='*', color='dimgrey')
#plt.scatter(over_voltages, energy_resolutions_dcr, label='DCR',s=size_marker)
plt.axhline(y=special_resolutions['max']['charge'], color='r', linestyle='--', label='Max', linewidth=1.5)

plt.axhline(y=special_resolutions['typical']['charge'], color='darkviolet', linestyle='--', label='Typical', linewidth=1.5)
plt.scatter(over_voltages, energy_resolutions_pde_analytical, label='PDE binomial',s=size_marker, marker='^',facecolors='none', edgecolors='green')
plt.scatter(over_voltages, energy_resolutions_pde_sqrtN,label='PDE $\\sqrt{N}$',s=size_marker, marker='^', facecolors='none',  edgecolors='deepskyblue')
plt.scatter(over_voltages, energy_resolutions_ct_analytical, label='crosstalk Borel',s=size_marker, marker='^', facecolors='none', edgecolors='crimson')
plt.scatter(over_voltages, energy_resolutions_ct_dec, label='crosstalk',s=size_marker, marker='s',color='darkcyan',facecolors='none', edgecolors='darkcyan')
