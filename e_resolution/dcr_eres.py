...
for s_file in special_files:
    ...
    resolutions = calculate_energy_resolution(f.Get("hist_dcr"), key, f.Get("hist_PDE"), True)
...
for i,file in enumerate(root_files):
    ...
    mean_pde /= pde
    mean_dcr /= pde
    energy_resolution_dcr = np.sqrt(sigma_dcr**2 - sigma_pde**2) / mean_dcr
...
# Plotting
plt.scatter(over_voltages, energy_resolutions_dcr, label='DCR',s=size_marker, marker='o', color='mediumblue')
...
plt.grid(True)
plt.savefig("energy_res_dcr.pdf")
