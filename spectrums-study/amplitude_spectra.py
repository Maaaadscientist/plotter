...
amp_min = data.min()
#nBins = 800
amp_step = 0.1#(amp_max - amp_min) / nBins
nBins = int((amp_max - amp_min) / amp_step)
print(nBins)
amp_gain = 1.5/ 2 / 5.28 * (vol + 2)
...
if not channel_data.empty:
    ...
    print(f"No data found for Channel {pos}")
...
plt.title('Maximum Amplitude Spectrum (LED Signal Range)')
plt.legend()
