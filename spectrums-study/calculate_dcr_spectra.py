...
for branch in ['baselineQ_ch']:
    ...
    initial_guess_mean = peak_positions[0]
    initial_guess_amplitude = 20
    initial_guess_std = 5
    # Fit the function to the histogram data
    popt, pcov = curve_fit(gaussian, bin_centers, hist_counts, p0=[initial_guess_mean, initial_guess_amplitude, initial_guess_std])
    fitted_mean, fitted_amplitude, fitted_std = popt
    bl_events = abs(fitted_amplitude * fitted_std * np.sqrt(2 * np.pi) / (bin_centers[1] - bin_centers[0]))
    all_events = np.sum(hist_counts)
    print(all_events, bl_events)
    dcr_events = all_events - bl_events
    mu_dcr = -np.log(bl_events/all_events)
    
    dcr_rate = dcr_events / (all_events * 360 * 1e-9) / 144
    print(dcr_rate)
    plt.plot(bin_centers, gaussian(bin_centers, *popt), label='Baseline Events')
    ...
    param_text = (f"SN: {batch}-{box}-{int(tsn)}-{channel}\n"
                  ...
                  f"DCR events (ch): {dcr_events:.0f}\n"
                  f"total time: {all_events * 1e-9 * 360:.5f}s\n"
                  f"DCR (this method): {dcr_rate:.0f}" +" ($\\mathrm{Hz}/\\mathrm{mm}^2$)\n"
                  "DCR $\\mu$: " + f"{mu_dcr:.2f}"
                  )
                  #"Charge Gain :"+f" {avg_gain:.2f}"+" $\\pm$ "+f"{avg_gain_err:.2f} (pC)\n"
    if branch != 'baselineQ_ch':
        param_text += "\nCharge Gain :"+f" {avg_gain:.2f} (pC)"
    plt.text(0.7, 0.7, param_text, transform=plt.gca().transAxes, verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5), fontsize=textsize)
    ...
    if branch == "baselineQ_ch":
        plt.errorbar(spectrum_x, spectrum_y, yerr=errors, fmt='o', color='black', ecolor='black', label='Dark Data', alpha = 0.7, zorder = 1)
        #plt.plot(spectrum_x, smoothed_data, label='Smoothed Dark', color='orange', zorder=2)
    else:
        plt.errorbar(spectrum_x, spectrum_y, yerr=errors, fmt='s', color='red', ecolor='red', label='Signal Data', alpha = 0.7, zorder = 1)
        #plt.plot(spectrum_x, smoothed_data, label='Smoothed Signal', color='purple', zorder=2)
...
plt.axvline(x=baseline_position + gain, color='orange', linestyle=':', linewidth=3, label='1 p.e.')  # red dashed line at x=5
plt.axvline(x=baseline_position + gain*2, color='deepskyblue', linestyle=':', linewidth=3, label='2 p.e.')   # green solid line at x=10
#plt.axvline(x=baseline_position + gain*3, color='purple', linestyle=':', linewidth=3, label='3 p.e.')   # green solid line at x=10
plt.title(f'Charge Spectrum of TSN-{tsn:.0f} Ch-{channel:.0f} at {vol+48}V')
plt.legend()
...
plt.ylim(1e-1,1e4)
plt.savefig(f"calculate_spectrum_tsn{tsn:.0f}_dark{vol}V.pdf")
plt.clf()
