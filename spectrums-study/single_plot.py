...
tsn = int(input_str.split(",")[0])
run = int(input_str.split(",")[1])
ch = int(input_str.split(",")[2])
vol = int(input_str.split(",")[3])

# Filter the run_pos_data for the current tsn
selected_data = run_pos_data[(run_pos_data['tsn'] == tsn) & (run_pos_data['run'] == run) & (run_pos_data['ch'] == ch) & (run_pos_data['vol'] == vol)]
pos = selected_data['pos'].iloc[0]
...
with open(script_name, 'w') as file:
    ...
    cmd = (f"$python {script_path}/{scr} {root_path}/main_run_{str(run).zfill(4)}/root/main_run_{str(run).zfill(4)}_ov_{vol}.00_sipmgr_{str(ch).zfill(2)}_tile.root "
           f"{run_pos_csv} {pos} {tsn}-{run}-{ch}-{vol}\n")
    file.write(cmd)
